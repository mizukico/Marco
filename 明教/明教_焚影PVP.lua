...
if MindwID == 0 then 
    ...
    SetTarget(TARGET.PLAYER, MindwID)  --设定目标为最近的敌对NPC                
end
end
...
if distance > 8 then if s_util.CastSkill(3970,false) then s_util.CastSkill(3975,false) return end end --幻光
...
--Z键隐身
if IsKeyDown("Z") then
    if s_util.CastSkill(3974,true) then return end
end

--F键滚键盘
if IsKeyDown("F") then
    
    if s_util.CastSkill(3967,false) then return end --破魔

	if s_util.CastSkill(3975,false) then return end --缴械
	
	if s_util.CastSkill(3979,false) then return end --驱夜
	
	if s_util.CastSkill(3960,false) then return end --月斩
	
	if s_util.CastSkill(3963,false) then return end --日斩
	
	if CurrentSun < 80 then 
        if s_util.CastSkill(3962,false) then return end --日轮
    end
	
	if CurrentMoon < 100 then 
        if s_util.CastSkill(3959,false) then return end --月轮
    end
end
...
if  CurrentSun >59 and CurrentSun <=79 and CurrentMoon >19 and CurrentMoon <=39 then
    if Is_B_in_A_FaceDirection(target, player, 180) then
    s_util.CastSkill(3970,false)
